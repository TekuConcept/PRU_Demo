...
{
    /*
    pin needs to be muxed before accessed otherwise nothing happens
    (this can also be accomplished through overlays - 
    though I have not yet successfully created one for this example)
    */
    if(!exists("/sys/class/gpio/gpio45/value")) // testing pin P8_11
        system("echo 45 > /sys/class/gpio/export;echo low > /sys/class/gpio/gpio45/direction");
    else
        system("echo low > /sys/class/gpio/gpio45/direction"); // set pin as output
